...
import src.coffe.tran_sizing as cur_tx_sizing 
...
def get_eval_delay(fpga_inst, opt_type, subcircuit, tfall, trise, low_voltage, is_ram_component, is_cc_component):
	...
	if opt_type == "local":
		...
		assert is_ram_component, "is_ram_component should be 1"
		# path_delay = 0
...
def write_sp_sweep_data_from_fpga(fpga_inst, sweep_out_fpath: str):
    """
...
def write_out_sweep_file(parameter_dict, sweep_out_fpath: str):
    """
...
def search_ranges(sizing_ranges, fpga_inst, sizable_circuit, opt_type, re_erf, area_opt_weight, 
	...
	tran_sizes_filename = ("sizes_" + sizable_circuit.name + 
	                      "_o" + str(outer_iter) + 
	                      "_i" + str(inner_iter) + 
	...
	# TODO: Turning this off for now
	export_filename = (sizable_circuit.name + 
	                  "_o" + str(outer_iter) + 
	                  "_i" + str(inner_iter) + 
	...
	for i in range(re_erf_num):
		...
		delay = cur_tx_sizing.get_final_delay(fpga_inst, opt_type, sizable_circuit, tfall, trise, is_ram_component, is_cc_component)
	...
	erf_export_filename = (sizable_circuit.name + 
	                      "_o" + str(outer_iter) + 
	                      "_i" + str(inner_iter) + 
