from src.buffer_dse_structs import *
import buffer_dse as bdse
...
def plot_c4_placements(c4_info: dict, design_info: dict):
    c4_grid_placement = GridPlacement(
        start_coord=GridCoord(c4_info["grid_margin"],c4_info["grid_margin"]),
        h=c4_info["diameter"],
        v=c4_info["diameter"],
        s_h=c4_info["pitch"],
        s_v=c4_info["pitch"],
...
def get_c4_placements_new(design_pdn: DesignPDN, pdn_dims: List[int]) -> dict:
...
def get_c4_placements(design_pdn: DesignPDN, pdn_dims: List[int]) -> Tuple[GridPlacement]:
    c4_grid_placement = GridPlacement(
        start_coord=GridCoord(design_pdn.c4_info.margin,design_pdn.c4_info.margin),
    ...
    pwr_rail_regions_grid_placement = GridPlacement(
...
def get_tsv_placements(design_pdn: DesignPDN, dims: List[int]) -> Tuple[GridPlacement]:
    ...
    if design_pdn.tsv_info.placement_setting == "dense":
        ...
        koz_grid = GridPlacement(
            start_coord = GridCoord(-design_pdn.tsv_info.single_tsv.keepout_zone, -design_pdn.tsv_info.single_tsv.keepout_zone),
        ...
        tsv_koz_outer_grid = GridPlacement(
            start_coord = GridCoord(-design_pdn.tsv_info.single_tsv.keepout_zone, -design_pdn.tsv_info.single_tsv.keepout_zone),
            h = design_pdn.tsv_info.single_tsv.diameter + design_pdn.tsv_info.single_tsv.keepout_zone*2,
            v = design_pdn.tsv_info.single_tsv.diameter + design_pdn.tsv_info.single_tsv.keepout_zone*2,
            s_h = design_pdn.tsv_info.single_tsv.pitch*2,
        ...
        if math.prod(dims) > 1:
            tsv_inner_grid = GridPlacement(
                start_coord=GridCoord((design_pdn.tsv_info.single_tsv.pitch),(design_pdn.tsv_info.single_tsv.pitch)),
                h=design_pdn.tsv_info.single_tsv.diameter,
                v=design_pdn.tsv_info.single_tsv.diameter,
                s_h=design_pdn.tsv_info.single_tsv.pitch*2,
                s_v=design_pdn.tsv_info.single_tsv.pitch*2,
            ...
            tsv_koz_inner_grid = GridPlacement(
                start_coord = GridCoord(design_pdn.tsv_info.single_tsv.pitch-design_pdn.tsv_info.single_tsv.keepout_zone, design_pdn.tsv_info.single_tsv.pitch-design_pdn.tsv_info.single_tsv.keepout_zone),
                h = design_pdn.tsv_info.single_tsv.diameter + design_pdn.tsv_info.single_tsv.keepout_zone*2,
                v = design_pdn.tsv_info.single_tsv.diameter + design_pdn.tsv_info.single_tsv.keepout_zone*2,
                s_h = design_pdn.tsv_info.single_tsv.pitch*2,
                s_v = design_pdn.tsv_info.single_tsv.pitch*2,
...
def tsv_calc(tsv_grids: dict) -> dict:
    ...
    koz_area = get_total_poly_area(KoZ_poly_bbs)
...
# def calc_PDN_metal_stack(metal_stack_info, tsv_info, beol_info, calc_tsv_grid, pwr_rail_distance):
#     # resistance from a single TSV grid to all of its power sinks
#     # Using direct via stack up to top two metal layers
#     # Add the resistance of the distance traveled on the top metal rail
#     # resitsance to get to the top two metal layers (M8 M9)
#     single_via_metal_stack_res = (sum(metal_stack_info["via_direct_stack_res_list"][0:len(metal_stack_info)-1]))
#     num_tsvs = calc_tsv_grid["dim"]**2 + (calc_tsv_grid["dim"]-1)**2
#     # top_metal_rail_res = beol_info["mlayer_ic_res_list"][len(beol_info["via_res_list"])-1]
#     # top metal layer tracks determines the resistance of top metal layer
#     # cross section of wire is HCw * (rail area?)
#     top_metal_rail_res = (tsv_info["resistivity"]*pwr_rail_distance)/(86.5*1e-6*(pwr_rail_distance))
#     # num via stacks going down to deliver power is another parameter
#     via_stacks_per_pwr_rail = 10
#     # conservatively assuming that all paths need to travel the maximum distance in top metal, in reality the 
#     single_c4_stack_res = calc_tsv_grid["resistance"] + single_via_metal_stack_res/num_tsvs + top_metal_rail_res + single_via_metal_stack_res/via_stacks_per_pwr_rail
#     print(f"single tsv resistance {tsv_info['resistance']}")
#     print(f"single via stack / num_tsvs {single_via_metal_stack_res/num_tsvs}")
#     print(f"(top_metal_res*pwr_rail_distance/num_tsvs): {top_metal_rail_res}")
#     print(f"single c4 stack resistance: {single_c4_stack_res} /36 -> {single_c4_stack_res/36}" )
#     num_c4s = 1
#     resistance = single_c4_stack_res
#     while resistance >= 0.1*1e-3:
#         resistance = single_c4_stack_res/num_c4s
#         num_c4s = num_c4s*2
#     print(f"resistance: {resistance}, c4_dim: {num_c4s}")




# def write_pdn_spice_model(design_pdn):
#     """ We want to be able to know what the resistance path from the ubmrella to a single current source is """
#     # inputs
#     # Metal Grid Info
#     # 
#     # 1. Num parallel metal tracks (pwr/gnd) per unit length
#     # 2. Transistor Pitch (using this as density for number of transistors in an area) -> This determines the size of the smallest metal grids
#     # 
    
#     pass


def get_res_info_from_dims(design_pdn: DesignPDN, dims: List[int], metal_rail_res_per_um: float, single_via_stack_res: float, pwr_rails_per_um: float, current_per_sq_um: float, region_info: dict, source: str):
...
def find_tsv_info(design_pdn: DesignPDN, in_dims: List[int], axis: int):
    """
    axis = 0 rows 
    """
    tsv_out_infos = []
    ...
    while True:
        ...
        tsv_rect_placements = PolyPlacements(
            rects = tsv_rects,
        ...
        koz_rect_placements = PolyPlacements(
            rects = koz_rects,
...
def get_via_stack_info(design_pdn: DesignPDN, via_stack_idx: int):
...
def shift_fpga_resouce(sector_pwr_region_grid: List[List[BlockRegion]], x_idx: int, y_idx: int, shift_dir: List[int], shift_amt: int):
    """
...
def generate_fpga_sectors(design_pdn: DesignPDN):
    """
    ...
    for l in bdse.get_df_output_lines(floorplan_out_df):
        print(l)
...
def calc_tsv_grid_to_top_metal_via_stack(design_pdn: DesignPDN, pwr_rail_pitch: float):
    """
...
def pdn_modeling(top_level_config: dict, process_info: ProcessInfo, ubump_infos: List[SolderBumpInfo] = None):
    ##################################################################################################
    # TODO sweep across all process infos
    # initialize the PDN object
    # make sure all info is in correct type
    design_dims = [float(dim) for dim in top_level_config["design_info"]["dims"]]

    pdn_sim_settings.plot_settings["tsv_grid"] = top_level_config["pdn_sim_settings"]["plot"]["tsv_grid"]
    pdn_sim_settings.plot_settings["c4_grid"] = top_level_config["pdn_sim_settings"]["plot"]["c4_grid"]
    pdn_sim_settings.plot_settings["power_region"] = top_level_config["pdn_sim_settings"]["plot"]["power_region"]
    pdn_sim_settings.plot_settings["pdn_sens_study"] = top_level_config["pdn_sim_settings"]["plot"]["pdn_sens_study"]


    design_pdn = DesignPDN(
        floorplan=sh.Polygon([(0,0), (0, design_dims[1]), (design_dims[0], design_dims[1]), (design_dims[0], 0)]),
        power_budget=float(top_level_config["design_info"]["power_budget"]), #W
        process_info=process_info, 
        supply_voltage=float(top_level_config["design_info"]["supply_voltage"]), #V
        ir_drop_budget=float(top_level_config["design_info"]["ir_drop_budget"]), #mV 
        fpga_info=FPGAInfo(
            sector_info=SectorInfo(), # unitized SectorInfo class to be calculated from floorplan and resource info
            sector_dims = top_level_config["design_info"]["fpga_info"]["sector_dims"],
            lbs = FPGAResource(
                total_num = int(top_level_config["design_info"]["fpga_info"]["lbs"]["total_num"]),
                abs_area = float(top_level_config["design_info"]["fpga_info"]["lbs"]["abs_area"]),
                rel_area = int(top_level_config["design_info"]["fpga_info"]["lbs"]["rel_area"]),
                abs_width = float(top_level_config["design_info"]["fpga_info"]["lbs"]["abs_width"]),
                abs_height = float(top_level_config["design_info"]["fpga_info"]["lbs"]["abs_height"]),
            ),
            dsps = FPGAResource(
                total_num = int(top_level_config["design_info"]["fpga_info"]["dsps"]["total_num"]),
                abs_area = float(top_level_config["design_info"]["fpga_info"]["dsps"]["abs_area"]),
                rel_area = int(top_level_config["design_info"]["fpga_info"]["dsps"]["rel_area"]),
            ),
            brams = FPGAResource(
                total_num = int(top_level_config["design_info"]["fpga_info"]["brams"]["total_num"]),
                abs_area = float(top_level_config["design_info"]["fpga_info"]["brams"]["abs_area"]),
                rel_area = int(top_level_config["design_info"]["fpga_info"]["brams"]["rel_area"]),
            )
        ),
        pwr_rail_info=PwrRailInfo(
            # pitch_fac=float(top_level_config["design_info"]["pwr_rail_info"]["pitch_fac"]),
            mlayer_dist = [float(top_level_config["design_info"]["pwr_rail_info"]["mlayer_dist"]["bot"]),float(top_level_config["design_info"]["pwr_rail_info"]["mlayer_dist"]["top"])],
            num_mlayers = int(top_level_config["design_info"]["pwr_rail_info"]["num_mlayers"]),
        ),
        tsv_info=TSVInfo(
            single_tsv=SingleTSVInfo(
                height=int(top_level_config["package_info"]["tsv"]["height"]), #um
                diameter=int(top_level_config["package_info"]["tsv"]["diameter"]), #um
                pitch=int(top_level_config["package_info"]["tsv"]["pitch"]), #um
                resistivity=float(top_level_config["package_info"]["tsv"]["resistivity"]), #Ohm*um (1.72e-8 * 1e6)
                keepout_zone=int(top_level_config["package_info"]["tsv"]["keepout_zone"]), # um     
                resistance=float(top_level_config["package_info"]["tsv"]["resistance"]), #Ohm
            ),
            area_bounds = top_level_config["design_info"]["pwr_placement"]["tsv_area_bounds"],
            placement_setting = top_level_config["design_info"]["pwr_placement"]["tsv_grid"],
            koz_grid=None,
            tsv_grid=None,
        ),
        c4_info=C4Info(
            SingleC4Info(
                height=int(top_level_config["package_info"]["c4"]["height"]), #um
                diameter=int(top_level_config["package_info"]["c4"]["diameter"]), #um
                pitch=int(top_level_config["package_info"]["c4"]["pitch"]), #um
                resistance=float(top_level_config["package_info"]["c4"]["resistance"]), #Ohm
                area=None,
            ),                
            placement_setting = top_level_config["design_info"]["pwr_placement"]["c4_grid"],
            margin=int(top_level_config["package_info"]["c4"]["margin"]), #um
            grid=None,       
        ),
        ubump_info=UbumpInfo(
            single_ubump=SingleUbumpInfo(
                height=float(top_level_config["package_info"]["ubump"]["height"]), #um
                diameter=float(top_level_config["package_info"]["ubump"]["diameter"]), #um
                pitch=float(top_level_config["package_info"]["ubump"]["pitch"]), #um
                # resistivity=float(top_level_config["package_info"]["ubump"]["resistivity"]), #Ohm*um (1.72e-8 * 1e6)    
            ),
            margin=float(top_level_config["package_info"]["ubump"]["margin"]),
            grid=None,
        )
    )
    ...
    tsv_out_infos_cols, out_dims = find_tsv_info(design_pdn, dims, axis = 0)
    tsv_out_infos_rows, out_dims = find_tsv_info(design_pdn, out_dims, axis = 1)

    tsv_out_infos = tsv_out_infos_cols + tsv_out_infos_rows
        
    design_pdn.tsv_info.dims = out_dims
    print("************************ CONSTANT INFO ************************")
    tsv_constants = {
        "Placement Parameter": design_pdn.tsv_info.placement_setting,
    }
    constant_out_df = pd.DataFrame(tsv_constants, index=[0])
    for l in bdse.get_df_output_lines(constant_out_df):
        print(l)
    print("************************ TSV GRID INFO ************************")
    tsv_output_df = pd.DataFrame(tsv_out_infos)
    for l in bdse.get_df_output_lines(tsv_output_df): 
        print(l)

    if pdn_sim_settings.plot_settings["tsv_grid"]:
        fig = go.Figure()
        design_pdn.tsv_info.koz_rect_placements.gen_fig(fig, fill_color = "red", opacity = 0.1)
        design_pdn.tsv_info.tsv_rect_placements.gen_fig(fig, fill_color = "blue", opacity = 0.5)
        fig.update_layout(
            title=f"{(design_pdn.tsv_info.placement_setting).upper()} TSV Grid Placement on a single C4 bump",
            xaxis_title="X (um)",
            yaxis_title="Y (um)",
    ...
    max_rails_per_um = 1 / (design_pdn.process_info.mlayers[-1].pitch * 2 * 1e-3)
    ...
    mlayer_usage = 1.00
    pitch_factor = 1
    pwr_rails_per_um = 1 / (design_pdn.process_info.mlayers[-1].pitch*pitch_factor*1e-3)
    ...
    while True:
        ...
        if mlayer_usage <= design_pdn.pwr_rail_info.mlayer_dist[-1]:
            break
        pitch_factor += 1

    pwr_rail_pitch = design_pdn.process_info.mlayers[-1].pitch*pitch_factor

    # Assume we need 2 metal layers for each set of PWR/GND power rails (X,Y directions)
    pwr_rails_per_um *= (design_pdn.pwr_rail_info.num_mlayers / 2)
    ...
    ###################################### VIA RES INFO ######################################
    via_grid_res = calc_tsv_grid_to_top_metal_via_stack(design_pdn, pwr_rail_pitch)
    ...
    """
    # current A / um ^ 2
    current_per_sq_um = (design_pdn.power_budget / design_pdn.supply_voltage) / (design_pdn.floorplan.area)
    ...
    for pitch, res in zip(ubump_pitches, ubump_resistances):
        single_ubump_info = SingleUbumpInfo(
            pitch = pitch,
            diameter = pitch/2,
            height = pitch/2,
            resistance=res,
        ...
        design_pdn.ubump_info.single_ubump = single_ubump_info
        design_pdn.update()
        ubump_out_infos = []
        ir_drop_out_infos = []
        # C4 Dimension Sweep
        ...
        top_metal_rail_res = design_pdn.process_info.mlayers[-1].wire_res_per_um
        ...
        while True:
            ...
            ubump_info = get_res_info_from_dims(design_pdn, ubump_dims, top_metal_rail_res, single_via_stack_res, pwr_rails_per_um, current_per_sq_um, region_info = None, source = "ubump") 
            c4_info = get_res_info_from_dims(design_pdn, c4_dims, top_metal_rail_res, single_via_stack_res, pwr_rails_per_um, current_per_sq_um, region_info = None, source = "c4") 
            ...
            c4_res = (design_pdn.c4_info.single_c4.resistance + design_pdn.tsv_info.resistance) + via_grid_res
            # ubump resistance in mOhm
            ubump_res = c4_res + (design_pdn.ubump_info.single_ubump.resistance*1e-3)
            
            # ceil as the only thing holding us back from using more microbumps is the metal resistance
            num_ubumps_per_pwr_region = math.ceil(( 1 /((design_pdn.ubump_info.single_ubump.pitch)**2))*design_pdn.tsv_info.tsv_rect_placements.bb_poly.area)
            ...
            design_pdn.c4_info.pdn_dims = c4_dims
            ...
            ubump_out_info["C4 Bump Res (Ohm)"] = design_pdn.c4_info.single_c4.resistance
            ubump_out_info["C4 Bump IR Drop (V)"] = design_pdn.c4_info.single_c4.resistance * c4_current_draw
            ...
            bot_ir_drop_info["C4 Bump"] = {
                "R (Ohms)": design_pdn.c4_info.single_c4.resistance,
                "I (mA)": round(c4_current_draw*1e3,4),
                "IR Drop (V)": round(design_pdn.c4_info.single_c4.resistance * c4_current_draw, 4),
                f"% of Total IR Drop": round((design_pdn.c4_info.single_c4.resistance * c4_current_draw) / bottom_die_ir_drop, 4),
            }
            bot_ir_drop_info["TSV Grid"] = {
                "R (Ohms)": design_pdn.tsv_info.resistance,
                "I (mA)": round(c4_current_draw*1e3,4),
                "IR Drop (V)": round(design_pdn.tsv_info.resistance * c4_current_draw, 4),
                f"% of Total IR Drop": round((design_pdn.tsv_info.resistance * c4_current_draw) / bottom_die_ir_drop, 4),
            }
            ...
            top_ir_drop_info["Micro Bump"] = {
                "R (Ohms)": design_pdn.ubump_info.single_ubump.resistance*1e-3,
                "I (mA)": round(ubump_info["current_per_crit_region"]*1e3,4),
                "IR Drop (V)": round(design_pdn.ubump_info.single_ubump.resistance * ubump_info["current_per_crit_region"], 4),
            ...
            design_pdn.pwr_region_dims = ubump_info["region_dims"]

            ir_drop_out_infos.append([ bot_ir_drop_info, top_ir_drop_info ])

            if max([bottom_die_ir_drop, top_die_ir_drop]) <= (design_pdn.ir_drop_budget*1e-3): #or dim > min(design_pdn.ubump_info.max_dims):
                break
        ...
        const_dict = [{
            ...
            "Single C4 Res (Ohms)": design_pdn.c4_info.single_c4.resistance*1e-3,
            "TSV Grid Res (Ohm)": design_pdn.tsv_info.resistance,
            "Single Via Stack Res (Ohms)": single_via_stack_res,
            "Total Via Stack Res (Ohms)": via_grid_res,
            "Single Ubump Res (Ohm)": design_pdn.ubump_info.single_ubump.resistance*1e-3,
        ...
        for l in bdse.get_df_output_lines(ubump_out_df):
            print(l)
        summary_out_info = {
            "Ubump Pitch": design_pdn.ubump_info.single_ubump.pitch,
        ...
        for value_type in unique_value_types:
            ...
            if pdn_sim_settings.plot_settings["pdn_sens_study"]:
                fig.show()
    ...
    for l in bdse.get_df_output_lines(summary_out_infos_df):
        print(l)
